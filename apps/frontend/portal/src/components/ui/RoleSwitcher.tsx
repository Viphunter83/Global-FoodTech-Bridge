...
export function RoleSwitcher() {
    ...
    const roles: { id: UserRole; label: string; icon: React.ReactNode }[] = [
        { id: 'MANUFACTURER', label: t('role_manufacturer'), icon: <Shield size={16} /> },
        { id: 'LOGISTICS', label: t('role_logistics'), icon: <Truck size={16} /> },
        { id: 'RETAILER', label: t('role_retailer'), icon: <ShoppingBag size={16} /> },
    ];

    return (
        <div className="flex items-center space-x-2 border-l border-gray-200 pl-4 ml-4">
            <span className="text-xs uppercase font-bold text-gray-500 mr-2 hidden md:inline-block">{t('role_persona')}</span>
